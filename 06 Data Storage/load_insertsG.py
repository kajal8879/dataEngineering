...
TempTableName = 'Temp_CensusData'
new_temp_buffers = "324MB"
...
def create_primaryKeyAndIndex(conn):
	with conn.cursor() as cursor:
		cursor.execute(f"""
...
def createTableFromTemp(conn):
	with conn.cursor() as cursor:
		cursor.execute(f"""
			DROP TABLE IF EXISTS {TableName};
			CREATE TABLE {TableName} AS SELECT * FROM {TempTableName}
		"""
		)

def increaseBufferMemory(conn):
	with conn.cursor() as cursor:
		cursor.execute("ALTER SYSTEM SET temp_buffers = '324MB'")
...
def main():
	...
	if CreateDB:
		...
		increaseBufferMemory(conn)
		createTempTable(conn)
		setTableUnlogged(conn)
		createTableFromTemp(conn)
	conn.autocommit = False
	load(conn, cmdlist)
	create_primaryKeyAndIndex(conn)
